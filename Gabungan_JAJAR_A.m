function [AA1,conflict_prop1,max_aircraftinsector, conflict_prop2, time, mean_time,ULT_CAP,max_conflictinsector,max_complexity,corelat, ya1, ya2, ya3, ya4, ya5] = Gabungan_JAJAR_A()
...
%                     [Distance_XY{1,i}(c1,c2),Distance_Altitude{1,i}(c1,c2),Distance_Merg{1,i}(c1,c2)] = separasi_jajar(Sim1{1,i}(2,c1),Sim1{1,i}(2,c2),Sim1{1,i}(3,c1),Sim1{1,i}(3,c2),Sim1{1,i}(4,c1),Sim1{1,i}(4,c2),Sim1{1,i}(27,c1),Sim1{1,i}(27,c2),Sim1{1,i}(1,c1),Sim1{1,i}(1,c2));
                    [Distance_XY{1,i}(c1,c2),Distance_Altitude{1,i}(c1,c2),Distance_Merg{1,i}(c1,c2)] = separasi_jajar_a(Sim1{1,i}(2,c1),Sim1{1,i}(2,c2),Sim1{1,i}(3,c1),Sim1{1,i}(3,c2),Sim1{1,i}(4,c1),Sim1{1,i}(4,c2),Sim1{1,i}(27,c1),Sim1{1,i}(27,c2),Sim1{1,i}(1,c1),Sim1{1,i}(1,c2),Sim1{1,i}(5,c1),Sim1{1,i}(5,c2),Sim1{1,i}(6,c1),Sim1{1,i}(6,c2));
                end
 ...
 %% Potential Conflict Definition              
                ...
                if Distance_Merg{1,i}(c1,c2) < buffer_merge && Distance_Merg{1,i}(c1,c2) ~= 0 % && Distance_Altitude{1,i}(c1,c2) <= meter(2000)
                    Conflict_3{1,i}(c1,c2) = 1;
                else
                    Conflict_3{1,i}(c1,c2) = 0;
                end
%% Conflict Definition              
                if (Distance_XY{1,i}(c1,c2) < conflict_separation_1  && Distance_XY{1,i}(c1,c2) ~= 0 && Distance_Altitude{1,i}(c1,c2) <= meter(1000))% || ((Distance_Merg{1,i}(c1,c2) < conflict_separation_1  && Distance_Merg{1,i}(c1,c2) ~= 0 && Distance_Altitude{1,i}(c1,c2) <= meter(1000)))
...
% % Conflict Definition              
                if (Distance_XY{1,i}(c1,c2) < conflict_separation_2  && Distance_XY{1,i}(c1,c2) ~= 0 && Distance_Altitude{1,i}(c1,c2) <= meter(1000)) % || ((Distance_Merg{1,i}(c1,c2) < conflict_separation_2  && Distance_Merg{1,i}(c1,c2) ~= 0 && Distance_Altitude{1,i}(c1,c2) <= meter(1000)))
                    Conflict_0{1,i}(c1,c2) = 1;
                else
                    Conflict_0{1,i}(c1,c2) = 0;
                end
...
%Conflict Colour
    for j = 1:airplane
        if mean(Conflict{1,i}(:,j)) > 0 || mean(Conflict_3{1,i}(:,j)) > 0
...
[Sim1{1,i+1}(9,j), Sim1{1,i+1}(31,j)]= clearance_a(ROW_Detail{1,i},Conflict{1,i},airplane,j,Conflict_3{1,i});
        % 0 : No clearance
        % 1 : Clearance
end
%Resolution (10)
...
if Sim1{1,i+1}(2,j) > 9000000
 ...
 Sim1{1,i+1}(24,j)= LOS(ROW_Detail{1,i},Conflict_1{1,i},airplane,j,Distance_XY{1,i},conflict_separation_1,conflict_separation_2,Sim1{1,i+1}(4,j)); % Lost of Separation
   ...
   Sim1{1,i+1}(30,j) = conflict_def(ROW_Detail{1,i},Conflict_0{1,i},airplane,j,Conflict_3{1,i}); %Status Conflict
   Sim1{1,i+1}(32,j) = clearance_a(ROW_Detail{1,i},Conflict{1,i},airplane,j,Conflict_3{1,i}); %Status Potential Conflict
...
if i==1
 ...
 if COMCTR{1,i+1}(1,j)== 1 
    if SLOTWL(1,i)== 0
  TCOM(1,i+1) = TCOM(1,i+1)+tcin; %time comm in
   SLOTWL(1,i+1)= 1;
...
ULT_CAP = round(max_aircraftinsector/mean_time*60);
...
ya1 = convertCharsToStrings(sprintf( '@%1d', sa1 ));
ya2 = convertCharsToStrings(sprintf( '@%1d', sa2 ));
ya3 = convertCharsToStrings(sprintf( '@%1d', sa3 ));
ya4 = convertCharsToStrings(sprintf( '@%1d', sa4 ));
ya5 = convertCharsToStrings(sprintf( '@%1d', sa5 ));
% ya6 = convertCharsToStrings(sprintf( '@%1d', sa6 ));
% ya7 = convertCharsToStrings(sprintf( '@%1d', sa7 ));
% ya8 = convertCharsToStrings(sprintf( '@%1d', sa8 ));