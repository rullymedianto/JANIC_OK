function [y, crossing] = clearance_b(ROW,conflict_xy,airplane,j,conflict_merge)
...
%            y = 1;
%           crossing =0;
       if conflict_xy(1,j) == 1 && conflict_merge(1,j) == 0  %&& distance(j,k) > buffer_zone && distancemerg (j,k)> buffer_zone
           y = 0;
           crossing =0;
      elseif  conflict_merge(1,j) == 1 %&& distance(j,k) > buffer_zone && distancemerg (j,k)> buffer_zone
           y = 0;
           crossing =1;
       else
           y=1;
