function [y, crossing] = conflicting(ROW,conflict_xy,airplane,j,conflict_merge)
...
%        k = it_2(1,column1-1);
       ...
       if it_2(2,column1-1) == 0 %&& distance(j,k) > buffer_zone && distancemerg (j,k)> buffer_zone 
           y = 1;
          crossing =0;
       elseif it_2(2,column1-1) == 1 && conflict_xy(1,j) == 1 %&& distance(j,k) > buffer_zone && distancemerg (j,k)> buffer_zone
           y = 0;
           crossing =0;
      elseif it_2(2,column1-1) == 1 && conflict_merge(1,j) == 1 %&& distance(j,k) > buffer_zone && distancemerg (j,k)> buffer_zone
           y = 0;
           crossing =1;
