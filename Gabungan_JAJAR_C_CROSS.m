...
if sched(1,i)==1
    ...
    Sim1{1,1}(33,i) = norm([Route{1,i}(5,1) Route{1,i}(5,2) Route{1,i}(5,3)]- [Sim1{1,1}(3,i) Sim1{1,1}(3,i) Sim1{1,1}(4,i)]);
                        if b == NavAIP{2,a}(1,1)
                            a = a+1;
                            b = 1;
                        elseif a+1>temp1
...
%ROW Definition
    for j = 1:airplane
        ROW{1,i}(1,j)= j;
        ROW{1,i}(2,j)= Conflict_1{1,i}(1,j); %Route{1,j}(Sim1{1,i}(1,j),1);
        ROW{1,i}(3,j)= Conflict_4{1,i}(1,j);%Route{1,j}(Sim1{1,i}(1,j),2);
        ROW{1,i}(4,j)= Sim1{1,i}(33,j);  
    end

%ROW Airplane
    for j = 1:airplane
        ROW_Detail{1,i} = Queueing1(ROW{1,i},airplane);
...
if Sim1{1,i+1}(2,j) > 9000000
 ...
 Sim1{1,i+1}(24,j)= LOS(ROW_Detail{1,i},Conflict_5{1,i},airplane,j,Distance_XY{1,i},conflict_separation_1,conflict_separation_2,Sim1{1,i+1}(4,j)); % Lost of Separation
  ...
  % Sim1{1,i+1}(31,j)= crossconf(ROW_Detail{1,i},Conflict_1{1,i},airplane,j,Conflict_4{1,i});%Potential crossing conflict
   ...
   Sim1{1,i+1}(33,j) = norm([Route{1,j}(5,1) Route{1,j}(5,2) Route{1,j}(5,3)]- [Sim1{1,i+1}(2,j) Sim1{1,i+1}(3,j) Sim1{1,i+1}(4,j)]);
...
% end
 ...
 if COMCTR{1,i+1}(1,j)== 1 
  if TCOM(1,i)>= TCOM(1,i+1)
    if    TCOM(1,i)< 2
        TCOM(1,i+1) = tcin; %time comm in
    elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tcin-1;
        COMDLY(1,i+1)=1;
        COMDLY(2,i+1)=TCOM(1,i)-1;   
    end
  else
      TCOM(1,i+1) = TCOM(1,i+1)+tcin-1;
        COMDLY(1,i+1)=1;
        COMDLY(2,i+1)=TCOM(1,i)-1+tcin; 
  end
 ...
 if COMCTR{1,i+1}(2,j)== 1
   if TCOM(1,i)>= TCOM(1,i+1)
     if    TCOM(1,i)< 2
        TCOM(1,i+1) = tcout; %time comm out
     elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tcout-1;
        COMDLY(1,i+1)=2;
        COMDLY(2,i+1)=TCOM(1,i)-1;
     end
   else
      TCOM(1,i+1) = TCOM(1,i+1)+tcout-1;
        COMDLY(1,i+1)=1;
        COMDLY(2,i+1)=TCOM(1,i)-1+tcout; 
   end
 end
  
 if COMCTR{1,i+1}(3,j)== 1 && Sim1{1,i+1}(31,j)==0
  if TCOM(1,i)>= TCOM(1,i+1)
     if TCOM(1,i)< 2
        TCOM(1,i+1) = tcvec; % vectoring and overtaking
     elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tcvec-1;
        COMDLY(1,i+1)=3;
        COMDLY(2,i+1)=TCOM(1,i)-1;
     end
  else
      TCOM(1,i+1) = TCOM(1,i+1)+tcvec-1;
      COMDLY(1,i+1)=1;
      COMDLY(2,i+1)=TCOM(1,i)-1+tcvec; 
  end 
 ...
 if COMCTR{1,i+1}(3,j)== 1 && Sim1{1,i+1}(31,j)==0
  if TCOM(1,i)>= TCOM(1,i+1)
     if    TCOM(1,i)< 2
        TCOM(1,i+1) = tccros; %crossing and vectoring
     elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tccros-1;
        COMDLY(1,i+1)=5;
        COMDLY(2,i+1)=TCOM(1,i)-1;
     end
  else
      TCOM(1,i+1) = TCOM(1,i+1)+tccros-1;
      COMDLY(1,i+1)=1;
      COMDLY(2,i+1)=TCOM(1,i)-1+tccros; 
  end    
 ...
 if COMCTR{1,i+1}(4,j)== 1 && Sim1{1,i+1}(31,j)==0
  if TCOM(1,i)>= TCOM(1,i+1)
    if    TCOM(1,i)< 2
        TCOM(1,i+1) = tccon; %overtaking and speed control
    elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tccon-1;
        COMDLY(1,i+1)=4;
        COMDLY(2,i+1)=TCOM(1,i)-1;
    end
  else
      TCOM(1,i+1) = TCOM(1,i+1)+tccon-1;
      COMDLY(1,i+1)=1;
      COMDLY(2,i+1)=TCOM(1,i)-1+tccon; 
  end  
 ...
 if COMCTR{1,i+1}(4,j)== 1 && Sim1{1,i+1}(31,j)==0
  if TCOM(1,i)>= TCOM(1,i+1)
     if TCOM(1,i)< 2
        TCOM(1,i+1) = tccros; %crosiing and speed control
     elseif TCOM(1,i)> 1
        TCOM(1,i+1) = TCOM(1,i)+tccros-1;
        COMDLY(1,i+1)=6;
        COMDLY(2,i+1)=TCOM(1,i)-1;
...
if (COMCTR{1,i+1}(1,j)== 0 ||COMCTR{1,i+1}(2,j)== 0 || COMCTR{1,i+1}(3,j)== 0 || COMCTR{1,i+1}(4,j)== 0)
    
    if j==1 && TCOM(1,i)>0
    TCOM(1,i+1) = TCOM(1,i)-1;
      
    elseif j~= 1 && TCOM(1,i+1)>0 
    TCOM(1,i+1) = TCOM(1,i+1);
